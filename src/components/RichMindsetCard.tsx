...
// 날짜 기반으로 매일 다른 카드가 노출됨. 홈 상단에서 사용자에게 부자/전설의 사고 패턴을 매일 주입.
// 철학: 1년 공부로 안 됨 → 매일 다른 각도로 같은 핵심을 반복.
...
interface MindsetCard {
  ...
  topicLabel: string;
  topicEmoji: string;
  headline: string;       // 핵심 메시지 한 줄
  body: string;           // 풀어쓴 설명 2-3 줄
  source?: string;        // 인용 출처
}
...
export function RichMindsetCard() {
  ...
  return (
    <section
      aria-label="오늘의 마인드셋"
      className="rounded-2xl border border-border bg-gradient-to-br from-primary/5 via-background to-background p-4 animate-fade-in"
    >
      <div className="flex items-center gap-2 mb-2">
        <span className="text-base">{card.topicEmoji}</span>
        <span className="text-xs font-bold tracking-wide text-primary">
          오늘의 마인드셋 · {card.topicLabel}
        </span>
      </div>
      ...
      {card.source && (
        <p className="text-[11px] text-muted-foreground mt-2 italic">— {card.source}</p>
      )}
    </section>
  );
}
