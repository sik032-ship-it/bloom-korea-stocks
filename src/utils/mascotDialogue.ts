...
export function getHomeGreeting(ctx: UserContext): MascotMessage {
 ...
 if (progress >= 90) {
 return { text: `거의 다 왔어요! \n다음 레벨까지 정말 조금 남았어요!\n오늘 레슨하면 진화할 수 있을지도...! `, mood: "thinking" };
 }
 if (ctx.streak >= 5) {
...
export function getWrongMessage(): string {
 const messages = [
 "아쉬워요! 하지만 이런 실수가 실전에서 돈을 지켜줘요 ",
 "괜찮아요! 여기서 배우면 실전에서 안 틀려요 ",
 "실수는 최고의 선생님이에요! 이 교훈이 위기 때 빛날 거예요 ",
...
export function getLessonMotivation(totalSentences: number, streak: number): string {
 ...
 const principles = [
 "FOMO를 느끼지 않는 것도 능력이에요. 오늘도 '나만의 기준'으로 생각해봐요 ",
...
export function getCompletionInsight(accuracy: number, isRepeat: boolean): string {
 ...
 // FOMO
 "FOMO가 없다는 건 '남의 수익에 흔들리지 않는다'는 뜻이에요. 이것만으로도 상위 10% 투자자예요 ",
 // 과거 vs 미래
 "10년 후에 '왜 그때 안 샀을까'라고 할 오늘의 하락이 있을 수 있어요. 미래 하락을 기회로 볼 수 있는 눈을 키우세요 ",
 // 시간 가치
...
export function getEmptyStateMessage(type: "holdings" | "sentences" | "archive"): { title: string; subtitle: string; emoji: string } {
 switch (type) {
 case "holdings":
 return { emoji: "", title: "아직 보유 종목이 없어요", subtitle: "종목을 추가하면 맞춤 질문으로\n투자 마인드를 훈련할 수 있어요!" };
 case "sentences":
