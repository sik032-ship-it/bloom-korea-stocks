...
function VerificationSentScreen({ email, onBack }: { email: string; onBack: () => void }) {
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6 py-8 animate-fade-in">
      <div className="w-full max-w-sm text-center">
        ...
        <p className="text-xs text-muted-foreground mb-6">
          메일함에서 인증 링크를 클릭하면<br />가입이 완료돼요. (스팸함도 확인해주세요)
        </p>
        <PpuriButton fullWidth onClick={onBack}>로그인 화면으로</PpuriButton>
      </div>
    </div>
  );
}
